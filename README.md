...
Bounded smoke comparison: 10 calibration cases per configuration, then 20 normal and 20 hard cases, with 3 samples and 4 concurrent tickets. These are small synthetic subsets, not the full benchmark. Cloud was disabled; the payload-byte comparison estimates what would be sent if enabled.

<!-- RESULTS:START -->

_Generated by `scripts/update_readme_results.py` from `reports/`. Synthetic team-written tickets; threshold and gate mode were chosen on the calibration split and locked before scoring._

| Run | Dataset | Samples | Category accuracy (95% CI) | Rules-only baseline | Kept on site | Error among kept | Unsafe accepts | p50 latency | PII leaked | Potential payload bytes per ticket (hybrid / cloud-only) |
|---|---|---|---|---|---|---|---|---|---|---|
| gpu-baseline | test.jsonl | 3 | 95.0% (76.4%–99.1%) | 75.0% | 0.0% | — | 0 | 14.9 s (4 in flight) | 0/4 | 4,613 / 6,242 |
| gpu-full | test.jsonl | 3 | 85.0% (64.0%–94.8%) | 75.0% | 15.0% | 0.0% | 0 | 10.3 s (4 in flight) | 0/4 | 130 / 4,196 |
| gpu-baseline | test_hard.jsonl | 3 | 90.0% (69.9%–97.2%) | 75.0% | 0.0% | — | 0 | 14.9 s (4 in flight) | 0/0 | 3,033 / 6,299 |
| gpu-full | test_hard.jsonl | 3 | 85.0% (64.0%–94.8%) | 75.0% | 5.0% | 0.0% | 0 | 10.3 s (4 in flight) | 0/0 | 213 / 4,370 |
...
```bash
cd ~
git clone --branch main https://github.com/abhinitasanabada-web/edgehack.git edgehack-integrated
